...
public class Simulator {
	...
	public int doOneExperiment(int size, double loadfactor, int mod) {
	    ...
	    Book.hashtype=Book.Hashtype.ADLER32;
	    //insert 1000 random books
	    for (int i = 0; i < 1000; i++) {
	    	Book b = getRandomBook();
	    	duplicates += table.insert(b.author, b);
	    }
	    
	    //lookup 1000 random books
	    for (int i = 0; i < 1000; i++) {
	    	Book b = getRandomBook();
	    	table.lookup(b.author);
	    }
	...
	public static void main(String[] args) {
		...
		sim.doOneExperiment(100, .90, 0);
		sim.doOneExperiment(100, .50, 0);
		
		sim.doOneExperiment(89, .75, 1);
		sim.doOneExperiment(89, .90, 1);
		sim.doOneExperiment(89, .50, 1);
